...
```{r}
#SecondFloor <- AmesData2$SecondSF > 0
#AmesData2$SecondSF <- SecondFloor
AmesData2$TotalPorch <- AmesData2$OpenPorchSF + AmesData2$EnclosedPorchSF + AmesData2$ScreenPorchSF
AmesData2$sqrt_LotArea <- sqrt(AmesData2$LotArea)
AmesData2$sqr_BasementfinSF <- AmesData2$BasementFinSF^2

plot(Price ~ . , data= AmesData2)

newmodel <- lm(log(Price) ~ Quality + FirstSF + SecondSF + sqr_BasementfinSF + 
    sqrt_LotArea + YearBuilt + GarageSF + BasementSF + Fireplaces + Condition + TotalPorch, 
    data = AmesData2)
summary(newmodel)
```
...
rstandard_data2 <- abs(rstandard(newmodel)) > 3
sum(abs(rstudent(newmodel)) > 2)
rstudent_data2 <- abs(rstudent(newmodel)) > 2
AmesData3 <- AmesData1[!rstandard_data2, ]
